...
set.seed(seed=as.numeric(argus[3]))
```
...
){stop("missing integrated samples")}
...
p2 <- FeaturePlot(s_obj,features=c("pseudobulk_msi"))+
  ggtitle('Pseudobulk sensor RNA score')+sc

p3 <- FeaturePlot(s_obj_cancer,features=c("sensor_rna_prob"),label = TRUE)+
  ggtitle('Recluster of cancer cells')+sc
p4 <- FeaturePlot(s_obj_cancer,features=c("pseudobulk_msi"))+
...
clusters <- DimPlot(s_obj,label=TRUE)+ ggtitle("All clusters")

kable_styling(kable(sample_info))

grid.arrange(clusters,cell_types,nrow=1,ncol=2)
grid.arrange(p1[[1]],p2[[1]],p3[[1]],p4[[1]],ncol=2,nrow=2)
